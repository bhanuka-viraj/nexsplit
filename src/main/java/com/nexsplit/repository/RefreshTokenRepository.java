...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, String> {
    ...
    /**
     * Count unique sources (IP + User Agent combinations) in a family
     * More efficient than Java stream processing for large families
     */
    @Query("SELECT COUNT(DISTINCT CONCAT(rt.ipAddress, '|', rt.userAgent)) FROM RefreshToken rt " +
    ...
    /**
     * Check if family has multiple sources (optimized version)
     * Returns true if more than 1 unique source exists
     */
